...
/// `out.len()` must be >= `ciphertext.len() / 16 * 16`. Hot-path core shared
/// by the sequential and pool paths (no allocation).
fn decrypt_blocks_into(tmpl: &Template, ciphertext: &[u8], out: &mut [u8]) {
    ...
    for bi in 0..nblocks {
        ...
        let mid = round1_mid(ctx, &mut st, ciphertext, &regs);
        let r2 = round1_tail(
            ctx,
            &mut st,
            mid.rax,
        ...
        let r2v = round2_sub6400(
            ctx, &mut st, r2.rdi, r2.rsi, r2.rdx, r2.rcx, r2.r8, r2.r9, r2.rax, r2.rbx,
        ...
        reconstruct_block(&r3v.pt, &mut out[bi * 16..bi * 16 + 16]);
